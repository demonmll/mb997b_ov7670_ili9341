...
{    	 
  ...
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_3 | GPIO_Pin_5| GPIO_Pin_6| GPIO_Pin_7| GPIO_Pin_8| GPIO_Pin_9| GPIO_Pin_10| GPIO_Pin_4;//LED0和LED1对应IO口
...
void LCD_Writ_COLORBus(u8 VH,u8 VL)   //颜色数据写入总线
{
  DATAOUT(VH);	
	LCD_WR_CLR;
	LCD_WR_SET;
...
void LCD_WR_DATAX(u16 data)
{
	LCD_RS_SET;
	LCD_CS_CLR;
	LCD_Writ_COLORBus(data>>8, data & 0xff);
	LCD_CS_SET;
...
{ 
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_All;	//  PORTE推挽输出
...
{          
	...
	u16 xlen=0;
		...
		for(i=sy;i<=ey;i++)
		{
			...
			LCD_WriteRAM_Prepare();     			//开始写入GRAM	  
			for(j=0;j<xlen;j++)LCD_WR_DATAX(color);	//设置光标位置 	    
		}
...
{  
 	...
 	for(i=0;i<height;i++)
	{
		...
		LCD_WriteRAM_Prepare();     //开始写入GRAM
		for(j=0;j<width;j++)LCD_WR_DATAX(color[i*width+j]);//写入数据 
	}	  
} 
//画线
//x1,y1:起点坐标
